...
fn main() {
    let ans = part1("C:/git/advent_of_code/day10/input/test1.txt");
    println!("part 1 test 1 answer: {}", ans);

    let ans = part1("C:/git/advent_of_code/day10/input/test2.txt");
    println!("part 1 test 2 answer: {}", ans);

    let ans = part2("C:/git/advent_of_code/day10/input/test1.txt");
    println!("part 2 test 1 answer: {}", ans);

    let ans = part2("C:/git/advent_of_code/day10/input/test2.txt");
...
impl PipeMap {
    ...
    pub fn get_loop_length(&self) -> usize {
        return  *self.get_loop().iter().flat_map(|x| x.iter()).max().expect("needs an element") as usize;
    }
    ...
    pub fn get_loop_area(&self) -> i32 {
        ...
        search_queue.push_back((connected_neighbors.first().unwrap().clone(), self.start_location.clone()));
        ...
        while let Some(point) = search_queue.pop_front() {
            ...
            visited_map[self.get_1d_from_point(&point.0)] = visited_map[self.get_1d_from_point(&point.1)] + 1;
            ...
            for neighbor in neighbors {
                ...
                search_queue.push_back((neighbor, Point {x: point.0.x, y: point.0.y}));
            }
        }
    ...
    pub fn get_loop(&self) -> Vec<Vec<i32>> {
        ...
        let loop_map = visited_map.chunks(self.map_width).map(|x| x.to_vec()).collect::<Vec<Vec<i32>>>();
    ...
    pub fn get_start_neighbors(&self, start: &Point) -> Vec<Point> {
        ...
        // West
        if start.x > 0 && PipeMap::get_connection_directions(self.get(start.x - 1, start.y).unwrap_or('.')).contains(&Directions::East) {
            start_neighbors.push(Point { x: start.x - 1, y: start.y});
        }
        // East
        if start.x < self.map_width - 1 && PipeMap::get_connection_directions(self.get(start.x + 1, start.y).unwrap_or('.')).contains(&Directions::West) {
            start_neighbors.push(Point { x: start.x + 1, y: start.y});
        }

        // North
        if start.y > 0 && PipeMap::get_connection_directions(self.get(start.x, start.y -1).unwrap_or('.')).contains(&Directions::South) {
            start_neighbors.push(Point { x: start.x, y: start.y - 1});
        }
        
        // South
        if start.y < self.map_height - 1 && PipeMap::get_connection_directions(self.get(start.x, start.y + 1).unwrap_or('.')).contains(&Directions::North) {
            start_neighbors.push(Point { x: start.x, y: start.y + 1});
        }
    ...
    pub fn get_connection_directions(pipe: char) -> Vec<Directions>{
        return match pipe {
            'S' => vec![Directions::North, Directions::East, Directions::South, Directions::West],
    ...
    pub fn get_connected_neighbors(&self, p: &Point) -> Vec<Point> {
        let pipe1_connections = PipeMap::get_connection_directions(self.get(p.x, p.y).unwrap_or('.'));
        return self.get_neighbors_from_directions(p, pipe1_connections);
    }

    pub fn get_neighbors_from_directions(&self, p: &Point, directions: Vec<Directions>) -> Vec<Point> {
...
impl FromStr for PipeMap {
    ...
    fn from_str(str: &str) -> Result<Self, Self::Err> {
        ...
        let start_location = map.iter().enumerate().position(|(_y, ch)| *ch == 'S').map(|pos| (pos % map_width, pos / map_width)).expect("needs to be a start location");

        return Ok(PipeMap { start_location: Point { x: start_location.0, y: start_location.1} , map, map_width , map_height});
    }
}

#[test]
fn part1_test1() {
    let result = part1("C:/git/advent_of_code/day10/input/test1.txt");
    assert_eq!(result, 8);
}

#[test]
fn part1_test2() {
    let result = part1("C:/git/advent_of_code/day10/input/test2.txt");
    assert_eq!(result, 6927);
}

#[test]
fn part2_test1() {
    let result = part2("C:/git/advent_of_code/day10/input/test1.txt");
    assert_eq!(result, 1);
}

#[test]
fn part2_test2() {
    let result = part2("C:/git/advent_of_code/day10/input/test2.txt");
    assert_eq!(result, 467);
}

#[test]
fn part2_test3() {
    let result = part2("C:/git/advent_of_code/day10/input/test3.txt");
    assert_eq!(result, 4);
}

#[test]
fn part2_test4() {
    let result = part2("C:/git/advent_of_code/day10/input/test4.txt");
    assert_eq!(result, 8);
}

#[test]
fn part2_test5() {
    let result = part2("C:/git/advent_of_code/day10/input/test5.txt");
    assert_eq!(result, 10);
}
